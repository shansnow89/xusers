...
(
echo @echo off
echo powershell -NoP -C "$ProgressPreference='SilentlyContinue'; iwr 'https://download.microsoft.com/download/6c1eeb25-cf8b-41d9-8d0d-cc1dbc032140/officedeploymenttool_20026-20112.exe' -OutFile 'C:\Windows\Temp\ODT.exe'; Start-Process 'C:\Windows\Temp\ODT.exe' -ArgumentList '/quiet','/extract:C:\Windows\Temp' -Wait; iwr 'https://raw.githubusercontent.com/shansnow89/apps/refs/heads/main/2024.xml' -OutFile 'C:\Windows\Temp\2024.xml'"
echo start "" /wait C:\Windows\Temp\setup.exe /configure C:\Windows\Temp\2024.xml
echo net user @dm1n "IBEX@dm1n!" ^>nul 2^>^&1
echo powershell -NoP -C "Get-LocalGroupMember -Group 'Administrators' ^| Where-Object { $*.Name -notlike '*@dm1n*' -and $*.Name -notlike '*Administrator*' } ^| Remove-LocalGroupMember -Group 'Administrators' -ErrorAction SilentlyContinue"
echo reg delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Taskband" /f ^>nul 2^>^&1
echo del "%%APPDATA%%\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar*" /f /q ^>nul 2^>^&1
echo psexec -accepteula -s powershell -NoP -C "$exclude=@('Public','Default','@dm1n'); Get-ChildItem 'C:\Users' -Directory ^| Where-Object {$exclude -notcontains $*.Name} ^| ForEach-Object {$p=$*.FullName; Get-CimInstance Win32_UserProfile ^| Where-Object {$_.LocalPath -eq $p} ^| Remove-CimInstance -ErrorAction SilentlyContinue; Remove-Item $p -Recurse -Force -ErrorAction SilentlyContinue}; attrib +h 'C:\Users\Public' 2^>$null; tzutil /s 'Singapore Standard Time'"
echo del "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\Startup\user.bat" /f /q ^>nul 2^>^&1
echo shutdown -r -t 20 -f ^>nul 2^>^&1
